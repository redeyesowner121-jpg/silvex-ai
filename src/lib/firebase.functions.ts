...
export const getFirebaseConfig = createServerFn({ method: "GET" }).handler(async () => {
  ...
  const ORIGIN = "silvex-ai";
  const projectId = env("FIREBASE_PROJECT_ID", ORIGIN);
  // Values below belong to the original store. Point FIREBASE_PROJECT_ID at a
  // different project and only that project's own settings are used.
  const own = projectId === ORIGIN;
  return {
    ...
    messagingSenderId: env("FIREBASE_MESSAGING_SENDER_ID", own ? "343959828375" : ""),
    appId: env("FIREBASE_APP_ID", own ? "1:343959828375:web:6d2990763b1f3ea8b576d2" : ""),
    measurementId: env("FIREBASE_MEASUREMENT_ID", own ? "G-B1MPW2N31F" : ""),
  };
});
